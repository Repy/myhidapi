...
static VALUE rb_wcstombs(wchar_t *str) {
    char buf[BUF_SIZE];
    int len;
...
static VALUE enumerate(VALUE mod, VALUE vendor_id, VALUE product_id) {
    ...
    while (cur_dev) {
        rb_ary_push(devices,
                    rb_funcall(mMyHIDAPI, rb_intern("build_device_info"), 8,
                               INT2NUM(cur_dev->vendor_id), INT2NUM(cur_dev->product_id),
                               rb_str_new2(cur_dev->path), rb_wcstombs(cur_dev->serial_number),
                               rb_wcstombs(cur_dev->manufacturer_string),
                               rb_wcstombs(cur_dev->product_string), INT2NUM(cur_dev->usage),
